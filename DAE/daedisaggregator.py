...
class DAEDisaggregator(Disaggregator):
    ...
    def _create_model(self, sequence_len, method):
        ...
        kernel_initializer=initializers.RandomNormal(mean=0.5, stddev=0.1)))
        model.add(Flatten())

        # Fully Connected Layers
        model.add(Dropout(0.1))
        model.add(Dense((sequence_len-0)*8, activation='relu',
        kernel_initializer=initializers.RandomNormal(mean=0.5, stddev=0.1)))#provare con linear sui dense
